...
interface UBrowser {
  ...
  */
  ...
  /**
   * 在一个闲置的 zbrowser 实例上运行。
   *
   * @param zbrowserId zbrowser 实例 ID：
   * - 可由 `run(options)` 在窗口未销毁时返回
   * - 或通过 `ztools.getIdleUBrowsers()` 获取
   */
 ...
 */
   ...
   */
  ai(
    option: ZToolsAiOption,
    streamCallback: (chunk: ZToolsAiMessage) => void
  ): ZToolsAiResult<void>
  /**
   * 非流式调用 AI
   * @param option 选项参数
